...
public List<BoardList> selectCommentByboardid(String boardid){
	SqlSession sqlSession = getSqlSessionFactory().openSession();
	
	try{
		return sqlSession.selectList(namespace2+".selectCommentByboardid",boardid);
	}finally{
		sqlSession.close();
	}
}	
...
public int countAllbyboardid(String boardid){
	...
	try{
		return (int) sqlSession.selectOne(namespace+".countAllbyboardid",boardid);
	}finally{
		sqlSession.close();
	}
}	
